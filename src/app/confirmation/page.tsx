...
function ConfirmationPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FAFAF8] pb-10">
      {/* Hero */}
      <div className="bg-[#2C2C2A] relative overflow-hidden pt-16 pb-8 px-5 text-center">
        <canvas ref={canvasRef} className="absolute inset-0 w-full h-full pointer-events-none" />
        <div className="relative z-10">
          <div className="w-16 h-16 rounded-full border-2 border-[#1D9E75] flex items-center justify-center mx-auto mb-4">
            <div className="w-12 h-12 rounded-full bg-[#1D9E75] flex items-center justify-center animate-pulse">
          ...
          <p className="text-[12px] text-[#888780]">{kitchen?.name} · {plan.charAt(0) + plan.slice(1).toLowerCase()} plan confirmed</p>
        </div>
      </div>

      {/* Stats */}
      <div className="flex border-b border-[#E8E5DE] bg-white">
        {[
          ...
          <div key={label} className="flex-1 text-center py-3 border-r border-[#E8E5DE] last:border-r-0">
            <p className={clsx('text-[14px] font-medium', green ? 'text-[#1D9E75]' : 'text-[#2C2C2A]')}>{val}</p>
      ...
      <div className="px-5 py-4 space-y-4">
        {/* Upcoming deliveries */}
        <div>
          <p className="text-[11px] font-medium text-[#888780] tracking-wider mb-2.5">UPCOMING DELIVERIES</p>
          <div className="card p-3.5">
            <div className="flex items-center gap-2.5 pb-3 mb-3 border-b border-[#F1EFE8]">
              ...
              <div>
                <p className="text-[13px] font-medium text-[#2C2C2A]">First delivery</p>
                <p className="text-[11px] text-[#888780]">{nextDates[0] ? fmt(nextDates[0]) : 'This week'} · 12–1pm</p>
              </div>
            </div>
            <div className="flex gap-2">
              {nextDates.map((d, i) => (
                <div key={i} className="flex-1 bg-[#f4fbf8] border border-[#9FE1CB] rounded-xl py-2 text-center">
        ...
        <div className="border border-[#1D9E75] rounded-2xl p-4 relative overflow-hidden">
          <div className="absolute -top-4 -right-4 w-20 h-20 rounded-full bg-[#E1F5EE] opacity-60" />
          ...
          <h3 className="text-[14px] font-medium text-[#2C2C2A] mb-1">Set your nutrition goals</h3>
          ...
          <div className="flex flex-wrap gap-1.5 mb-3">
            {GOALS.map(g => (
              <button key={g} className="px-2.5 py-1 rounded-full text-[11px] font-medium border border-[#E8E5DE] text-[#5F5E5A] bg-white hover:bg-[#2C2C2A] hover:text-white hover:border-[#2C2C2A] transition-all">
                {g}
              </button>
            ))}
          </div>
          <Link href="/planner">
            <button className="w-full py-2.5 bg-[#1D9E75] text-white rounded-xl text-[13px] font-medium">
              Personalise my plan →
            </button>
          </Link>
      ...
      <div className="px-5">
        <Link href="/">
          <button className="w-full py-3.5 bg-[#2C2C2A] text-white rounded-2xl text-[13px] font-medium flex items-center justify-center gap-2">
            <Home size={15} />
            Back to home
          </button>
        </Link>
