  ...
  return (
    <div className="min-h-screen bg-[#F5F5F0] pb-24">
      <div className="px-5 pt-14 pb-6">
        <h1 className="font-serif text-[22px] text-[#1A3A2A] mb-6">Profile</h1>

        {/* Avatar + info */}
        <div className="flex items-center gap-4 mb-6">
          <div className="w-16 h-16 rounded-full bg-[#FFFBEB] flex items-center justify-center text-[20px] font-medium text-[#1A3A2A] flex-shrink-0">
            {initials}
          </div>
          <div>
            <p className="text-[16px] font-medium text-[#1A3A2A]">{user?.name ?? 'Welcome!'}</p>
            <p className="text-[13px] text-[#8A9A8A]">{user?.email ?? 'Sign in to access your account'}</p>
            {user && <p className="text-[11px] text-[#1A3A2A] mt-0.5">Vancouver, BC</p>}
          </div>
        </div>
        ...
        {user && (
          <div className="grid grid-cols-3 gap-2 mb-6">
            {[
              ...
              <div key={label} className="bg-white border border-[#D8DDD0] rounded-2xl p-3 text-center">
                <p className="text-[18px] font-medium text-[#1A3A2A]">{val}</p>
        ...
        {user ? (
            ...
            {MENU_ITEMS.map(({ icon: Icon, label, href }, i) => (
              <Link key={href} href={href}>
                <div className={`flex items-center gap-3 px-4 py-3.5 ${i < MENU_ITEMS.length - 1 ? 'border-b border-[#F1EFE8]' : ''}`}>
                  ...
                  <span className="flex-1 text-[13px] text-[#1A3A2A]">{label}</span>
          ...
          <div className="space-y-3 mb-4">
            <Link href="/auth/login">
              <button className="w-full py-3.5 bg-[#1A3A2A] text-white rounded-2xl text-[14px] font-medium">
                Sign in
              </button>
            </Link>
            <Link href="/auth/signup">
              <button className="w-full py-3.5 bg-white border border-[#D8DDD0] text-[#1A3A2A] rounded-2xl text-[14px] font-medium">
                Create account
              </button>
            </Link>
          </div>
        ...
        {user && (
          <button
            ...
            className="w-full py-3 border border-[#D8DDD0] bg-white rounded-2xl text-[13px] font-medium text-[#D85A30] flex items-center justify-center gap-2"
          >
            <LogOut size={14} />
            Sign out
          </button>
