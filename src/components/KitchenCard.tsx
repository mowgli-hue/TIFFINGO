      ...
      <div className="card overflow-hidden hover:shadow-md transition-shadow duration-200">
        ...
        <div
          className={clsx(
            ...
            kitchen.type === 'tiffin' ? 'bg-[#FFF0E6]' : 'bg-[#FAEEDA]'
          )}
        >
          ...
          <div className="absolute top-2 left-2 flex gap-1.5">
            {kitchen.type === 'tiffin' && (
              <span className="bg-[#C8522A] text-white text-[9px] font-medium px-2 py-0.5 rounded-full">
                Plan
              </span>
            )}
            {kitchen.isHalal && (
              <span className="bg-white text-[#5F5E5A] text-[9px] font-medium px-2 py-0.5 rounded-full border border-[#E8DDD0]">
                Halal
              </span>
            )}
          </div>
          ...
          {/* Rating */}
          <div className="absolute top-2 right-2 bg-white rounded-lg px-2 py-0.5 flex items-center gap-1 border border-[#E8DDD0]">
          ...
          {!kitchen.isOpen && (
            <div className="absolute inset-0 bg-white/70 flex items-center justify-center">
              <span className="text-[11px] font-medium text-[#9A8A7A]">Closed now</span>
            </div>
          )}
        </div>
        ...
        {/* Body */}
        <div className="p-3">
          <h3 className="text-[13px] font-medium text-[#2C1810] mb-0.5">{kitchen.name}</h3>
          <p className="text-[11px] text-[#9A8A7A] mb-2">{kitchen.cuisine} · {kitchen.type === 'tiffin' ? 'Home kitchen' : 'Restaurant'}</p>
